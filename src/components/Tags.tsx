    ...
    <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm transition-all hover:shadow-md">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-bold text-stone-400 bg-stone-100 px-2 py-0.5 rounded-md font-mono">
            {taskTags.length} tags
          </span>
        </div>
        <div className="flex flex-wrap gap-2">
          {taskTags.map((tag) => (
            <Pill key={tag} variant="task" className="px-2.5 py-1 text-xs font-semibold rounded-md border-emerald-100">
              {tag}
            </Pill>
          ))}
        </div>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm transition-all hover:shadow-md">
        ...
        <div className="flex flex-wrap gap-2">
          {methodTags.map((tag) => (
            <Pill key={tag} variant="method" className="px-2.5 py-1 text-xs font-semibold rounded-md border-amber-100">
              {tag}
            </Pill>
          ))}
        </div>
